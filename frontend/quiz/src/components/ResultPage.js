...
class ResultPage extends React.Component {
  ...
  render(){
    ...
    for(i=0; i< parseInt(this.props.numberOfQuestions, 10); i++){
      var color;
      if(this.props.quiz[i].correctAnswer === this.props.quiz[i].chosenAnswer ){
        color = 'correct'}
        else{
          color = 'incorrect'
        }
       
       resultList.push(<div key = {i} className = {color}>
        
        <div><h3>QUESTION {i+1}</h3>{this.props.quiz[i].question}</div>
